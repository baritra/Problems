...
public class Universe {
    ...
    public int minHacking(String s, int maxDamage) {
        Map<Integer, Integer> strenghts = strengthAtIndices(s);
        int hacks = 0;
        int totalDamage = 0;
        for (int damage : strenghts.values()) {
            totalDamage += damage;
        }
        ...
        int i = s.length() - 1;
        while (i >= 0) {
            if (instructions[i] == 'S' && i >= 1 && instructions[i-1] == 'C') {
                hacks++;
                char temp = instructions[i];
                instructions[i] = instructions[i-1];
                instructions[i-1] = temp;

                int strengthAtIndex = strenghts.get(i);
                strenghts.put(i - 1, strengthAtIndex/2);
                strenghts.remove(i);

                totalDamage -= strengthAtIndex/2;
                if (totalDamage <= maxDamage) {
                    return hacks;
                }
    ...
    private Map<Integer, Integer> strengthAtIndices(String s) {
        int sum = 0;
        int strength = 1;
        Map<Integer, Integer> strengths = new HashMap<>();
        for (int i = 0; i < s.length(); i ++ ) {
            if (s.charAt(i) == 'S') {
                strengths.put(i, strength);
            } else {
                strength *= 2;
            }
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0 ; i < numTests ; i++) {
            ...
            int maxDamage = lineScanner.nextInt();
            String instructions = lineScanner.next();
            int hacks = universe.minHacking(instructions, maxDamage);
            answers.add("Case #" + (i+1)+": "+(hacks >= 0 ? hacks+"" : "IMPOSSIBLE"));
        }
